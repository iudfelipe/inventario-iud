...
import { getMarcaById, createMarca, updateMarca } from '../../api/helper';

function CreateOrEditMarca() {
  const { id } = useParams();
  const isEdition = !!id;
  ...
  const onSubmit = async (isEdition) => {
    const apiCall = isEdition ? updateMarca : createMarca;
  ...
  useEffect(() => {
    (async function () {
      if (isEdition && id) {
        try {
          setIsLoading(true);
          const { data: { data } } = await getMarcaById(id);
  ...
  return (
    <main>
      <h1 className="mb-5">{isEdition ? 'Editar' : 'Crear'} Marca</h1>
      {isLoading && (
...
export default CreateOrEditMarca;
